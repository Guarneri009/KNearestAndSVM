...
/** @function main */
int main_digit(int argc, char** argv)
{
    ...
    Mat trainData = Mat(cv::Size(classes * train_samples, ImageSize), CV_32FC1, cv::Scalar(0));
    Mat trainClasses = Mat::zeros(cv::Size(classes * train_samples, 1), CV_32FC1);
...
void RunSelfTest(KNearest& knn2)
{
    ...
    while (z++ < 10)
    {
        ...
        //OLD code
        //float detectedClass = knn2.findNearest(sample2, 1);
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        ...
        if (contourArea(cnt) > 50)
        {
            ...
            if (rec.height > 28)
            {
                ...
                //code
                //float result = knearest.findNearest(sample2, 1);
                float result = 0;
